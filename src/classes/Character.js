...
export class Character extends Entity {
    ...
    // Set difficulty settings for damage calculations
    ...
    takeDamage(amount, source = null) {
        ...
        // Apply difficulty-based damage reduction and cap for player
        // (Little Bitch Mode: reduced boss damage + damage cap)
        if (this.difficultySettings && source) {
            const diff = this.difficultySettings.difficulty ?? 1.0;
            
            if (diff < 1.0) {
                // Check if source is a boss
                const isBossAttack = source.isBoss || source.isCoreBoss;
                
                if (isBossAttack) {
                    // Scale damage: at diff 0.0 = 50%, at diff 1.0 = 100%
                    const damageMult = 0.5 + (diff * 0.5);
                    amount = Math.floor(amount * damageMult);
                    
                    // Apply damage cap in Little Bitch Mode (diff < 0.25)
                    // Cap at 40% of max health
                    if (diff < 0.25) {
                        const maxDamage = Math.floor(this.maxHealth * 0.4);
