...
export class PauseMenu {
    constructor(canvas, game) {
        ...
        this.game = game;
        this.visible = false;
        this.activeTab = 'controls'; // 'controls', 'visuals', 'audio', 'gameplay'
        ...
        this.settings = {
            ...
            particles: true,
            // Gameplay
            difficulty: 1.0, // 1.0 = Normal, 0.0 = Little Bitch Mode
            marketsEnabled: true,
    ...
    applySettings() {
        ...
        this.game.gameplaySettings = {
            difficulty: this.settings.difficulty,
            marketsEnabled: this.settings.marketsEnabled
        };
    ...
    render(ctx) {
        ...
        // Tab buttons
        const tabs = ['controls', 'visuals', 'audio', 'gameplay'];
        const tabW = 120;
        const tabH = 35;
        ...
        if (this.activeTab === 'controls') {
            ...
            this.renderGameplayTab(ctx, panelX, contentY, panelW);
        }
    ...
    renderGameplayTab(ctx, panelX, startY, panelW) {
        // Difficulty slider - from Normal (1.0) to Little Bitch Mode (0.0)
        const diffY = startY + 30;
        const diffValue = this.settings.difficulty;
        
        // Difficulty label with mode name
        ctx.fillStyle = '#cccccc';
        ctx.font = '16px Arial';
        ctx.textAlign = 'left';
        ctx.fillText('Difficulty', panelX + 40, diffY);
        
        // Mode name based on difficulty
        const modeName = diffValue >= 0.75 ? 'NORMAL' : 
                         diffValue >= 0.25 ? 'EASY' : 'LITTLE BITCH MODE';
        const modeColor = diffValue >= 0.75 ? '#ff6666' : 
                          diffValue >= 0.25 ? '#ffff66' : '#ff88ff';
        ctx.fillStyle = modeColor;
        ctx.font = 'bold 14px Arial';
        ctx.textAlign = 'right';
        ctx.fillText(modeName, panelX + panelW - 30, diffY);
        
        // Difficulty slider track
        const sliderX = panelX + 40;
        const sliderW = panelW - 120;
        const sliderY = diffY + 25;
        
        // Track background
        ctx.fillStyle = '#2a2a4a';
        ctx.fillRect(sliderX, sliderY - 4, sliderW, 8);
        
        // Gradient fill from pink to red
        ...
        ctx.fillStyle = gradient;
        ctx.fillRect(sliderX, sliderY - 4, sliderW * diffValue, 8);
        
        // Handle
        const handleX = sliderX + sliderW * diffValue;
        ctx.fillStyle = modeColor;
        ctx.beginPath();
        ctx.arc(handleX, sliderY, 10, 0, Math.PI * 2);
        ctx.fill();
        
        this.sliders.push({ x: sliderX, y: sliderY, w: sliderW, key: 'difficulty' });
        ...
        ctx.textAlign = 'left';
        if (diffValue < 0.25) {
            ctx.fillText('Bosses deal 50% damage, have 50% HP, damage capped at 40% of your HP', panelX + 40, sliderY + 30);
        } else if (diffValue < 0.75) {
            ctx.fillText('Bosses deal 75% damage, have 75% HP', panelX + 40, sliderY + 30);
        } else {
            ctx.fillText('Full difficulty - git gud or git rekt', panelX + 40, sliderY + 30);
        }
        
        // Markets toggle
        const marketsY = sliderY + 70;
        const marketsEnabled = this.settings.marketsEnabled;
        
        ctx.fillStyle = '#cccccc';
        ctx.font = '16px Arial';
        ctx.textAlign = 'left';
        ctx.fillText('Market Floors', panelX + 40, marketsY + 5);
        ...
        ctx.fillStyle = marketsEnabled ? '#3a6a3a' : '#5a3a3a';
        ctx.fillRect(btnX, marketsY - 12, btnW, btnH);
        ctx.strokeStyle = marketsEnabled ? '#5a9a5a' : '#8a5a5a';
        ctx.lineWidth = 2;
        ctx.strokeRect(btnX, marketsY - 12, btnW, btnH);
        
        ctx.fillStyle = '#ffffff';
        ctx.font = 'bold 14px Arial';
        ctx.textAlign = 'center';
        ctx.fillText(marketsEnabled ? 'ON' : 'OFF', btnX + btnW / 2, marketsY + 7);
        
        this.buttons.push({ x: btnX, y: marketsY - 12, w: btnW, h: btnH, action: 'toggle', key: 'marketsEnabled' });
        
        // Markets description
        ctx.fillStyle = '#888888';
        ctx.font = '12px Arial';
        ctx.textAlign = 'left';
        ctx.fillText(marketsEnabled ? 'Shop between floors' : 'Skip directly to next combat floor', panelX + 40, marketsY + 35);
    }
}
